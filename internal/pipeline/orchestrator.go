...
func Execute(action string, payload map[string]string, onProgress ProgressFunc) (map[string]string, error) {
	...
	switch action {
	case "deploy":
		return executeDeploy(payload, notify)
	case "restart":
		return executeRestart(payload, notify)
	case "status":
		return executeStatus(payload, notify)
	default:
...
func executeDeploy(payload map[string]string, notify func(step string, message string, meta map[string]string)) (map[string]string, error) {
	...
	if workDir != "" && repoURL == "" {
		...
		if output, err := runCommand("", "git", "-C", workDir, "pull", "--ff-only"); err != nil {
	...
	if testCmd != "" {
		...
		if output, err := runShell(testCmd, workDir); err != nil {
	...
	if deployCmd := strings.TrimSpace(payload["deploy_cmd"]); deployCmd != "" {
		...
		if output, err := runShell(deployCmd, workDir); err != nil {
	...
	if composeFile != "" {
		...
		if output, err := runCommand(workDir, "docker", args...); err != nil {
...
func executeRestart(payload map[string]string, notify func(step string, message string, meta map[string]string)) (map[string]string, error) {
...
func executeStatus(payload map[string]string, notify func(step string, message string, meta map[string]string)) (map[string]string, error) {
	repoDir := strings.TrimSpace(payload["repo_dir"])
